	...
	internal static class InterfaceTests
	{
       ...
       ///  Test an ICollection that should contain the given values, possibly in order.
        ...
        public static object SerializeRoundTrip(object objToSerialize)
        {
            ...
            formatter.Serialize(streamOut, objToSerialize);
            byte[] objBinaryData = streamOut.ToArray();

            // Deserialize the object from its binary data representation
            MemoryStream streamIn = new MemoryStream(objBinaryData);
            object result = formatter.Deserialize(streamIn);
